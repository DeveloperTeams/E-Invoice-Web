...
function YoloDetectionIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
      <rect x="3" y="3" width="18" height="18" rx="2" stroke="#F59E0B" strokeWidth="2" strokeDasharray="4 2" />
      <circle cx="12" cy="12" r="3" fill="#F59E0B" />
    </svg>
  );
}

interface InvoiceYoloResultProps {
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
      ...
      <div className="mb-8 flex flex-col gap-4 rounded-2xl border border-border/70 bg-surface-alt/65 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
        <div className="flex flex-col gap-2">
          ...
          <h2 className="text-2xl font-bold text-text sm:text-3xl">Invoice Data Extracted (YOLO)</h2>
        </div>
        <button
          onClick={onReset}
      ...
      {processingInfo.bounding_box && (
        <div className="mb-6">
          <ResultCard icon={<YoloDetectionIcon />} title="YOLO Detection Region" fullWidth>
            <InfoRow
              label="Bounding Box"
              value={
                <span className="text-sm">
                  x: {processingInfo.bounding_box.x}, y: {processingInfo.bounding_box.y},
                  w: {processingInfo.bounding_box.width}, h: {processingInfo.bounding_box.height}
                </span>
              }
            />
            ...
            <InfoRow
              ...
              value={<Badge variant="warning">{processingInfo.detection_method}</Badge>}
            />
          </ResultCard>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        {invoiceData.llm_enhancement && invoiceData.llm_enhancement.enabled && (
          <ResultCard icon={<AIIcon />} title="AI Enhancement">
            ...
            />
            <InfoRow label="Model" value={<Badge>{invoiceData.llm_enhancement.model}</Badge>} />
            <InfoRow
              label="Validation"
              value={
            ...
            {invoiceData.llm_enhancement.debug_summary?.fields_changed !== undefined && (
              <InfoRow
                ...
                value={<Badge variant="purple">{invoiceData.llm_enhancement.debug_summary.fields_changed}</Badge>}
              />
            )}
          </ResultCard>
        )}
        ...
        <ResultCard icon={<ProcessingIcon />} title="Processing Info">
          ...
          <InfoRow label="Provider" value={processingInfo.provider || <EmptyValue />} />
